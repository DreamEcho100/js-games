...
<select
		...
		</div>`),document.getElementById(f)?.querySelectorAll("input[type='radio']").forEach(e=>{n.registerEventListener({elem:e,type:"change",listener:d=>{r=d.target.value,l=0}})});const m=document.getElementById("vanillaJavascriptSpriteAnimationTechniques");if(!m)throw new Error("Couldn't find the canvas!");const i=m.getContext("2d");if(!i)throw new Error("Couldn't get the canvas context!");const $=C({canvas:m,ctx:i,onUpdateCanvasSize:e=>{t=e}});n.register($);let x=5,l=0,c;function u(){let e=Math.floor(l/x)%a[r].size;const d=s*e,S=a[r].locations[e].y;i.clearRect(0,0,t.width,t.height),i.drawImage(o,d,S,s,g,0,0,t.width,t.height),l++,c=requestAnimationFrame(u)}n.register(()=>{c&&cancelAnimationFrame(c)}),u()}});export{F as default};
